...
- **Crossplane**: Use the dedicated `provider-opentofu` package for native execution.
- **Atlantis**: Native integration via config and binary installation.
- **Digger**: Native via `opentofu: true` in project config.
- **Terrateam**: Native support for workflows.
- **Terrakube**: Native selection per workspace.
