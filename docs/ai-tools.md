---
layout: default
title: AI Tools and Resources
---

# AI Tools, Frameworks, Agents
- [The Agency: AI Specialists - Transform Workflows](https://github.com/msitarzewski/agency-agents/) - prompt library, not a runtime orchestration framework
- [CrewAI](https://github.com/crewAIInc/crewAI/tree/main) - High-level opinionated framework designed for orachestraing role-playing autonomous AI agents
- [LangGraph](https://docs.langchain.com/oss/python/langgraph/overview) - is a low-leve, graph-based orchestration framework, treating AI workflows as directed graphs
-   [Llama 3.2 models](https://github.com/meta-llama/llama-models). central repo for OpenSource Llama 3.2 models (including both the text-only 1B/3B models and the multimodal Vision models).

## AI Models
-   [Claude Code Best Practices](https://github.com/shanraisshan/claude-code-best-practice)
-   [Llama 3.2 models](https://github.com/meta-llama/llama-models). central repo for OpenSource Llama 3.2 models (including both the text-only 1B/3B models and the multimodal Vision models)

# AI RAG (Retrieval, Agumented, Generation) 
-   [RAG from Scratch](https://github.com/langchain-ai/rag-from-scratch)

# Catholic AI
- [Magisterium](https://www.magisterium.com)
- [MagisAI, Created  by Fr. Robert Spitzer, SJ of Magis Center](https://www.magiscenter.com/magisai)
- [Truthly, Co-founded by Matt Fradd](https://www.truthly.ai/)
